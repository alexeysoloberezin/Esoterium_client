<template>
  <div class="card">
    <Menubar :model="items" class="p-2">
      <template #start>
        <svg width="35" height="40" viewBox="0 0 35 40" fill="none" xmlns="http://www.w3.org/2000/svg" class="h-2rem mr-3">
          <path
      ...
      <template #item="{ item, props, hasSubmenu, root }">
        <RouterLink :to="item.path" v-ripple class="flex align-items-center mx-3 px-3 py-2" v-bind:class="{ 'text-primary': $route.path === item.path }" v-bind="props.action">
...
const items = computed(() => {
  return isAdmin.value ? itemsAdmin : itemsStudent
})
</script>
